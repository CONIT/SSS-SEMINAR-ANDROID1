...
public class ResponseHandler {
     ...
     * 
     * @param pendingIntent a PendingIntent that we received from Android Market
     *            that will create the new buy page activity
    ...
    public static void purchaseResponse2(final Context context, final String productId,
            final String receipt, final long purchaseTime) {
        // Update the database with the purchase state. We shouldn't do that
        // from the main thread so we do the work in a background thread.
        // We don't update the UI here. We will update the UI after we update
        // the database because we need to read and update the current quantity
        ...
        new Thread(new Runnable() {
            public void run() {
                PurchaseDatabase db = new PurchaseDatabase(context);
                db.updatePurchase(productId, receipt, purchaseTime);
                db.close();
                ...
                synchronized (ResponseHandler.class) {
                    if (sPurchaseObserver != null) {
                        sPurchaseObserver.postPurchaseStateChange(productId, receipt, purchaseTime);
                    }
                }
            }
        }).start();
     ...
     *            response code
     * @param responseCode a response code from Market to indicate the state of
     *            the request
     */
