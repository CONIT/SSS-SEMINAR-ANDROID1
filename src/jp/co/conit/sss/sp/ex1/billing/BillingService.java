...
public class BillingService extends Service implements ServiceConnection {
    ...
    private void purchaseStateChanged(int startId, String signedData, String signature) {
        ...
        for (VerifiedProduct vp : purchases) {
            ...
            ResponseHandler.purchaseResponse2(this, vp.getProductId(), vp.getReceipt(),
                    vp.getPurchaseTime());
        }
    ...
    /**
     * SamuraiPurchaseで注文情報の検証（nonce、signature）をします。
     */
    class VrifyOrderThread extends Thread {
        ...
        public void run() {
            super.run();
            mVerifyPurchase = Security.verifyPurchase(getApplicationContext(), mSignedData,
                    mSignature);
        }
    }
    ...
    /**
     * リストア処理かどうかを判定します。<br>
     * 購入情報にnotificationIDがあるかないかで判定しています。<br>
     * 
     * @param
     * @return
     */
    ...
    private void notificationVerifiedProduct(List<VerifiedProduct> purchases) {
        ...
        // リストの最後尾（最新のもの）からMapにデータを投入し、同一プロダクトが存在する場合の古い購入データを排除する
        ...
        // 購入完了プロダクトが１つ以上の場合はNotificationで通知する
        // プロダクト毎に通知を行う場合は要修正
        if (orderMap.size() > 0) {
            ...
            notification.setLatestEventInfo(context,
                    context.getString(R.string.notification_buy_finish, title),
                    context.getString(R.string.notification_urge_download), pendingintent);
